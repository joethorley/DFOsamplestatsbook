...
Under a strict interpretation of the frequentist framework they should tell their manager that the sex ratio is skewed towards males (the p-value for the null hypothesis of an equal sex ratio is $0.5^5 = 0.031$) and that their best estimate is that all 10,000 individuals are males!
...
Indeed given this much prior information then they would require 68 or more males from a sample size of 100 fish to be surprised if sex ratio wasn't skewed.

```{r}
set.seed(42)
rate_effect_bayesian(32, 100, alpha = 101, beta = 101)
```
This is exactly the same result as if we had no prior knowledge about sex ratio and sampled 132 males out of 300 fish.
```{r}
set.seed(42)
...
Surprisal (in the Bayesian framework) is how surprising (in terms of tossing x heads in a row on a fair coin) it would be to discover the that true effect is in the opposite direction to the estimated effect.
